...
M.insertEnter = function()
  if vim.fn.executable("im-select") ~= 1 and vim.fn.executable("im-select.exe") ~= 1 then
    vim.notify("No im-select and unable to switch IMs, https://github.com/daipeihust/im-select")
    return
  end
...
M.insertLeave = function()
  if vim.fn.executable("im-select") ~= 1 and vim.fn.executable("im-select.exe") ~= 1 then
    vim.notify("No im-select and unable to swith IMs, https://github.com/daipeihust/im-select")
    return
  end
  if vim.fn.has("macunix") == 1 then
    macInsertLeave()
